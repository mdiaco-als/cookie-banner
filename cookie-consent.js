...
(function () {
  ...
  try {
    ...
    var css = ""
      ...
      + "#cc-banner{padding:1rem;border-radius:16px;width:96vw;max-height:90vh}"
      + "#cc-banner h3{font-size:1.1rem;margin-bottom:1rem;line-height:1.2}"
      + "#cc-banner p{margin:.6rem 0;padding:8px;font-size:.85rem;line-height:1.3}"
      + "#cc-banner p svg{width:16px;height:16px}"
      + "#cc-actions{flex-direction:column;gap:8px;margin-top:1rem}"
      + "#cc-links{margin:1rem 0 .5rem;gap:8px;font-size:.8rem}"
      ...
      + "#cc-card{padding:1.2rem}"
      + ".cc-row{padding:12px;margin:8px 0}"
      + "}"
      + "@media (max-width:480px){"
      + "#cc-banner{padding:.8rem;width:98vw;max-height:88vh}"
      + "#cc-banner h3{font-size:1rem;margin-bottom:.8rem}"
      + "#cc-banner p{margin:.5rem 0;padding:6px;font-size:.8rem}"
      + "#cc-actions button{font-size:.85rem;padding:10px 12px}"
      + "#cc-links{flex-wrap:wrap;justify-content:center}"
      + "#cc-links a{font-size:.75rem;padding:3px 6px}"
      + "#cc-float{left:calc(10px + env(safe-area-inset-left));bottom:calc(10px + env(safe-area-inset-bottom));"
      + "width:52px;height:52px;padding:4px}#cc-float img{width:40px;height:40px}"
      + "#cc-card{padding:1rem;width:96%}"
      + ".cc-row{padding:10px;font-size:.9rem}"
      + ".cc-row input{transform:scale(1.2)}"
      + "}"
      + "@media (max-width:360px){"
      + "#cc-banner{padding:.6rem;border-radius:12px}"
      + "#cc-banner h3{font-size:.95rem;margin-bottom:.6rem}"
      + "#cc-banner p{margin:.4rem 0;padding:4px;font-size:.75rem}"
      + "#cc-actions button{font-size:.8rem;padding:9px 10px}"
      + "#cc-links{gap:4px}"
      + "#cc-links a{font-size:.7rem;padding:2px 4px}"
      + "#cc-float{width:48px;height:48px}#cc-float img{width:36px;height:36px}"
      + "#cc-card{padding:.8rem}"
      + ".cc-row{padding:8px;font-size:.85rem}"
      + "}"
      + "@media (max-height:600px) and (max-width:640px){"
      + "#cc-banner{max-height:95vh;overflow-y:auto}"
