...
(function () {
  ...
  try {
    ...
    var css = ""
      ...
      // Responsive modernizzato
      + "@media (max-width:640px){"
      + "#cc-banner{padding:1.5rem;border-radius:20px;width:95vw}"
      + "#cc-banner h3{font-size:1.2rem}"
      + "#cc-actions{flex-direction:column;gap:10px}"
      + "#cc-docs-card{width:100vw;height:100dvh;border-radius:0}"
      + "#cc-links{flex-direction:column;gap:8px}"
      + "#cc-float{width:60px;height:60px;padding:6px}#cc-float img{width:44px;height:44px}"
      + "}"
      + "@media (max-width:480px){"
      + "#cc-float{left:calc(12px + env(safe-area-inset-left));bottom:calc(12px + env(safe-area-inset-bottom));"
      + "width:56px;height:56px;padding:6px}#cc-float img{width:40px;height:40px}"
      + "#cc-actions button{font-size:.9rem;padding:12px 14px}"
      + "}"
      + "@media (max-width:430px){#cc-actions button{font-size:.85rem;padding:11px 12px}}"
      + "@media (max-width:380px){#cc-actions button{font-size:.8rem;padding:10px 10px}}"
      + "@media (max-width:360px){#cc-actions button{font-size:.8rem;padding:9px 8px}}";
